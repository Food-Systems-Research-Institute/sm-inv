...
# Temporary save for conference paper graph
# Add total indicator counts to indicator by year data
asfs <- latex_tab %>% 
  select(Dimension, Indicators) %>% 
  right_join(indicators_by_year_data, by = join_by(Dimension == dimension)) %>% 
  rename(total_indicators = Indicators) %>% 
  setNames(c(snakecase::to_snake_case(names(.))))
write_csv(asfs, "3_coverage/output/indicator_temporal_data.csv")
...
out <- asfs %>% 
  ...
  left_join(
    ...
    by = join_by(dimension == Dimension)
  ) %>% 
...
indicator_plot_alt <- out %>% 
  # Set orders in fill var for missing on top, counts below
  mutate(fill_var = factor(fill_var, levels = c(rbind(
    paste(names(dp_palette), "missing_count", sep = "_"),
    paste(names(dp_palette), "indicator_count", sep = "_")
  )))) %>% 
  ggplot(
    aes(x = year, y = count, fill = fill_var, group = fill_var)
  ) +
  geom_col(position = "stack") +
  scale_x_continuous(
    breaks = seq(
...
ggsave(
  indicator_plot_alt,
  filename = "3_coverage/output/fig_indicator_coverage_alt.png",
  width = 5,
  height = 4,
  units = "in",
  dpi = 300,
